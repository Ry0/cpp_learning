...
int sort_selector(student a[], int stu);//ソートする方法を選択させる関数
void bubble_sort(student a[], int stu);//バブルソートの関数
void bubble_sort_progress(student a[], int stu);//プログレスバー付きのバブルソート
void selection_sort(student a[], int stu);//選択ソートの関数
void selection_sort_progress(student a[], int stu);//プログレスバー付きの選択ソート
void merge_sort(student ary[], student temp[], int a, int b, int stu, int sub);//マージソートの関数
void merge(student ary[], student temp[], int a, int b, int c, int stu, int sub);//マージ用関数
...
int main(void){
  ...
  while(1){
    ...
    //temp = malloc(sizeof(student) * student_num);
    for(i=0;i<student_num;i++){
      data[i].score = malloc(sizeof(int) * subject_num);
      //temp[i].score = malloc(sizeof(int) * subject_num);
    }
  ...
  printf("\n");
    ...
    /*ソート*/
  sort_judge = sort_selector(data, student_num);
  if(sort_judge==0){
    if(student_num>=20000){
      printf("ソート中...\n");
        gettimeofday(&start, NULL);//時間計測の開始
        bubble_sort_progress(data, student_num);
    ...
    }else if(sort_judge==1){
      if(student_num>=20000){
        printf("ソート中...\n");
        gettimeofday(&start, NULL);//時間計測の開始
        selection_sort_progress(data, student_num);
        gettimeofday(&end, NULL);//時間計測の終了
        printf("ソートが完了しました．何か文字を入力してEnter．\n");
        scanf("%s", &key);
      }else{
        gettimeofday(&start, NULL);//時間計測の開始
        selection_sort(data, student_num);
        gettimeofday(&end, NULL);//時間計測の終了
      }
  }else if(sort_judge==2){
      temp = malloc(sizeof(student) * student_num);
      for(i=0;i<student_num;i++){
        temp[i].score = malloc(sizeof(int) * subject_num);
      }
      gettimeofday(&start, NULL);//時間計測の開始
      merge_sort(data, temp, 0, student_num-1, student_num, subject_num);
      gettimeofday(&end, NULL);//時間計測の終了
  }else{
      gettimeofday(&start, NULL);//時間計測の開始
      quick_sort(data, 0, student_num-1);
      gettimeofday(&end, NULL);//時間計測の終了
  }
...
void random_input(student a[], int stu, int sub){
  ...
  for(i=0;i<stu;i++){
    a[i].num = i+1;//生徒番号を1から順に番号つけ
    for(j=0;j<sub;j++){
      a[i].score[j] = rand()%101;//0から100までの乱数を発生
    }
  }
}
...
int sort_selector(student a[], int stu){
...
void bubble_sort_progress(student a[], int stu){
  int i,j;
  int progress;
  int grogress_count;
  int percent;
  ...
  for(i=0;i<stu-1;i++){
    ...
    if(progress==grogress_count-1){
      ...
      percent = (float)i/grogress_count * 5 + 1;
      printf("%4d ％",percent);
    }
    printf("\r");
  }
...
void selection_sort(student a[], int stu){
  int i,j;
  int low,key;
...
void selection_sort_progress(student a[], int stu){
  int i,j;
  int progress;
  int grogress_count;
  int percent;
...
void merge(student ary[], student temp[], int a, int b, int c, int stu, int sub){
  int i,j,k;
...
void merge_sort(student ary[], student temp[], int a, int b, int stu, int sub){
  int center;
  ...
  merge_sort(ary, temp, a, center, stu, sub);
  merge_sort(ary, temp, center+1, b, stu, sub);

  merge(ary, temp, a, b, center, stu, sub);
}
...
void quick_sort(student a[], int left, int right){
  ...
  pivot = a[(left + right)/2].average;

  while(1){
    while(a[i].average < pivot){
      i++;
    }

    while(pivot < a[j].average){
      j--;
    }
