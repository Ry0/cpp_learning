...
//     printf("ファイルをオープンできません。\ndata.datのファイルの保存場所をもう一回確認してください。\n");
//     return -1;
//   }else{
//     for(i=0;i<stu;i++){
//       fscanf(fp, "%d", &a[i].num);
...
// }
  ...
  /*データ入力*/
int input(student a[], int stu, int sub){
  int judge,retry,errer_counter=0;
  int i,j;

  while(1){
    if(3<=errer_counter){
      printf("最初に定義した生徒人数、教科数と外部ファイルの生徒人数、教科数が正しくない可能性があります。\nもう一回生徒人数、教科数を定義してください。\n");
      break;
    }else{
      ...
      if(judge==0){
        ...
        if(external_data_input(a, stu, sub)==-1){
          errer_counter++;
        }else{
          break;
        }
...
while(1){
  ...
  if(input(data, student_num, subject_num)<=2){
    break;
  }
}
