...
void merge_sort(student ary[], int a, int b, int stu, int sub);
...
int main(void){
  ...
  char key;
  student *data,*temp;//入力された生徒数分の点数データを確保する用
  ...
  while(1){
    ...
    if(sort_judge==0){
      ...
      if(student_num>=20000){
        ...
        gettimeofday(&start, NULL);//時間計測の開始
        merge_sort(data, 0, student_num-1, student_num, subject_num);
        gettimeofday(&end, NULL);//時間計測の終了
    }
...
void merge_sort(student ary[], int a, int b, int stu, int sub){
  ...
  temp = malloc(sizeof(student) * stu);
  for(i=0;i<stu;i++){
    temp[i].score = malloc(sizeof(int) * sub);
  }
  ...
  merge_sort(ary, a, center, stu, sub);
  merge_sort(ary, center+1, b, stu, sub);

  for(i=a;i<=center;i++){
    temp[i] = ary[i];
  }

  for(i=center+1,j=b;i<=b;i++,j--){
    temp[i] = ary[j];
  }
